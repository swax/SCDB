...
function ensureEnvironment({ handle }) {
  ...
  if (!TWITTER_USER_ID && !handle && !TWITTER_HANDLE) {
    console.error(
      "Error: Provide a Twitter handle via CLI or set TWITTER_HANDLE.",
    );
    process.exit(1);
  }
}
...
function serializeParams(params) {
  ...
  return flattened
    .map(
      ([key, value]) =>
        `${percentEncode(String(key))}=${percentEncode(String(value))}`,
    )
    .join("&");
}
...
function buildTweetsPath({ userId, paginationToken, limit }) {
  const params = new URLSearchParams({
    max_results: String(
      Math.max(Math.min(limit, MAX_PAGE_LIMIT), MIN_PAGE_LIMIT),
    ),
...
function renderTweets(tweets, { username }) {
  ...
  for (const tweet of tweets) {
    ...
    console.log(
      "Tweet URL:",
      resolveTweetLink({ tweetId: tweet.id, username }),
    );
