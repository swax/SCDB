...
if (nodeVersion < 18) {
  console.error(
    "Error: This script requires Node.js 18 or higher (for built-in fetch and FormData)",
  );
  process.exit(1);
}
...
function usage() {
  console.log(
    "Usage: node operations/sketchtvlol/upload-image.js <image_path> <table_name> <base_url> [--verbose]",
  );
  console.log(
    "Example: node operations/sketchtvlol/upload-image.js /path/to/image.jpg sketch https://www.sketchtv.lol",
  );
  console.log(
    "Example: node operations/sketchtvlol/upload-image.js /path/to/image.jpg sketch http://localhost:3000 --verbose",
  );
  console.log("");
  console.log("Options:");
...
async function uploadImage(imagePath, tableName, baseUrl, verbose = false) {
  try {
    ...
    const beginResponse = await makeRequest(
      `${baseUrl}/api/upload-image/begin`,
      {
        method: "POST",
        headers: {
    ...
    if (!beginResponse.success) {
      throw new Error(
        `Failed to get presigned URL: ${beginResponse.error || "Unknown error"}`,
      );
    }
    ...
    const finishResponse = await makeRequest(
      `${baseUrl}/api/upload-image/finish`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!finishResponse.success) {
      throw new Error(
        `Failed to create database record: ${finishResponse.error || "Unknown error"}`,
      );
    }
