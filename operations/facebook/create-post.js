...
if (process.argv.length < 4) {
  console.error(
    'Usage: node operations/facebook/create-post.js "Post text" "Page URL"',
  );
  console.error(
    'Example: node operations/facebook/create-post.js "Check out this sketch!" "https://sketchtv.lol/sketch/123"',
  );
  process.exit(1);
}
...
if (!FACEBOOK_PAGE_ACCESS_TOKEN) {
  console.error(
    "Error: FACEBOOK_PAGE_ACCESS_TOKEN environment variable is required",
  );
  process.exit(1);
}
...
async function debugToken(token) {
  if (!APP_ID || !APP_SECRET) {
    throw new Error(
      "FACEBOOK_APP_ID and FACEBOOK_APP_SECRET are required to debug tokens",
    );
  }
...
async function postToPage(accessToken, body, linkUrl) {
  ...
  const path = `/${GRAPH_VERSION}/${FACEBOOK_PAGE_ID}/feed?debug=all`;
...
async function main() {
  try {
    ...
    if (APP_ID && APP_SECRET) {
      ...
      console.log(
        "ℹ️ Provide FACEBOOK_APP_ID and FACEBOOK_APP_SECRET to print expiry info.",
      );
    }
