...
if (!APP_ID || !APP_SECRET) {
  console.error(
    "Error: FACEBOOK_APP_ID and FACEBOOK_APP_SECRET environment variables are required",
  );
  process.exit(1);
}
...
if (!USER_ACCESS_TOKEN) {
  console.error(
    "Error: FACEBOOK_USER_ACCESS_TOKEN environment variable is required",
  );
  process.exit(1);
}
      ...
      res.on("end", () => {
        ...
        try {
          ...
          const parseError = new Error(
            `Unable to parse response JSON: ${error.message}`,
          );
...
function formatExpiry(debugData = {}, fallbackExpiresIn) {
  const { expires_at: expiresAtSeconds, expires_in: expiresInSeconds } =
    debugData;
...
async function main() {
  try {
    console.log(
      "♻️ Exchanging short-lived user token for a long-lived token...",
    );
    ...
    console.log(
      "⏱️ User token expires:",
      formatExpiry(userDebug.data, exchange.expires_in),
    );
    ...
    if (!pagesResponse.data || pagesResponse.data.length === 0) {
      console.log(
        "No managed pages were returned. Check that the user token has the right permissions.",
      );
      return;
    }
    ...
    for (const page of pagesResponse.data) {
      ...
      if (!page.access_token) {
        console.log(
          "No access token returned for this page. Ensure the user has manage permissions.",
        );
        continue;
      }
      ...
      try {
        ...
        console.error(
          "⚠️ Unable to determine page token expiry:",
          debugError.message,
        );
      }
    }
