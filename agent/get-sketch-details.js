...
async function getSketchDetails(sketchId, withFolder = false) {
    ...
    try {
        ...
        details += `Description: ${sketch.description || 'N/A'}\n`;
        ...
        if (sketch.sketch_casts.length > 0) {
            ...
            sketch.sketch_casts.forEach((cast, index) => {
                ...
                if (cast.character_name || cast.character) {
                    ...
                    const characterId = cast.character_id ? ` (Character ID: ${cast.character_id})` : '';
        ...
        if (withFolder) {
            ...
            const year = sketch.season?.year || new Date(sketch.created_at).getFullYear();
