...
const getCachedPerson = cache(async (slug: string) => getPersonBySlug(slug));
...
export async function generateMetadata({
  ...
  return buildPageMeta(
    title,
    description,
    getContentPath("person", person.url_slug),
...
export async function generateStaticParams() {
  ...
  return people.list.map((person) => ({
    slug: person.url_slug,
  }));
}

export default async function PersonPage({ params }: SlugPageProps) {
  // Data fetching
  const person = await tryGetContentBySlug(params, getCachedPerson);
