...
const getCachedSketch = cache(async (slug: string) => getSketchBySlug(slug));
...
export async function generateMetadata({
  ...
  const sketch = await getCachedSketch(slug);
  if (!sketch) {
    return {};
  }
  ...
  return buildPageMeta(
    title,
    description,
    getContentPath("sketch", sketch.url_slug),
    images,
  );
}
...
export default async function SketchPage({ params }: SlugPageProps) {
  // Data fetching
  const sketch = await tryGetContentBySlug(params, getCachedSketch);
