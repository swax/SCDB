...
const getCachedShow = cache(async (slug: string) => getShowBySlug(slug));
...
export async function generateMetadata({
  ...
  const show = await getCachedShow(slug);
  if (!show) {
    return {};
  }
  ...
  return buildPageMeta(
    title,
    description,
    getContentPath("show", show.url_slug),
...
export default async function ShowPage({ params }: SlugPageProps) {
  // Data fetching
  const show = await tryGetContentBySlug(params, getCachedShow);
